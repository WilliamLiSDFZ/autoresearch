...
# ///
"""Compare complete autoresearch solutions using cosine Vendi."""
...
def load_samples(path):
    """Import saved solution summaries/vectors; reject legacy node representations."""
    unique = {}
    for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if row.get("stage") != "solution" or row.get("representation_version", "solution-v1") != "solution-v1":
            raise ValueError(f"Input line {number}: only solution-v1 samples with stage=solution are supported")
        card = row.get("mechanism_card")
        if "assessment_status" in row or (isinstance(card, dict) and ("status" in card or "change" in card)):
        ...
        row.setdefault("text", "")
        row.setdefault("representation_version", "solution-v1")
...
def coverage_rows(samples, issues):
    """Count all attempted solutions, including missing sources and failed training."""
    ...
    for key, rows in sorted(groups.items()):
        ...
        result.append(dict(zip(("study_id", "task", "run_id", "arm", "stage", "view"), key)) | {
            ...
            "n_missing": counts["missing_source"],
...
def score_samples(samples, baseline, repeats, seed):
    """Compute matched-count Vendi within isolated study/task/representation cohorts."""
    ...
    for key, rows in sorted(cohorts.items()):
        ...
        # Keep each study and source protocol separate for matched-count comparisons.
...
def extract_samples(samples, summarizer):
    """Summarize each complete source snapshot without trial scores or retrieval reports."""
    from autoresearch_vendi.runtime import SOLUTION_PROMPT
...
def plot_results(out, scores, comparisons):
    ...
    for cohort in sorted({row["cohort"] for row in scores}):
        ...
        note = ("Static implementation summaries. Bands show candidate-subset variation, not effect confidence intervals.\n"
...
def save_outputs(out, samples, issues, scores, comparisons, settings, identity, calls,
    ...
    with (out / "samples.jsonl").open("w", encoding="utf-8") as stream:
        for row in samples:
            # Keep source-grounded cards and vectors without copying raw code into samples.
            exported = {k: v for k, v in row.items() if k != "source"}
    ...
    charts = [] if prepare_only or no_plots else plot_results(out, scores, comparisons)
    ...
    measurement = "whole-candidate static solution summaries; no parent required"
    ...
    scope = ("Complete source snapshots are summarized independently using one neutral prompt. Initial and later "
             ...
             "descriptive; use matched-count scores for comparisons.")
    report = ["# Autoresearch solution Vendi analysis", "",
    ...
    report += ["", "See coverage.csv for source and extraction failures, solution_cards.csv for source-grounded summaries, "
...
def main(argv=None):
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "results/vendi-solutions", help="default: results/vendi-solutions")
    parser.add_argument("--cache", type=Path, default=ROOT / "results/vendi-solutions-cache", help="default: results/vendi-solutions-cache")
    ...
    parser.add_argument("--embedding-backend", choices=("local", "openai"), default="openai",
                        help="default: openai; use local for MiniLM embeddings")
    ...
    parser.add_argument("--prepare-only", action="store_true", help="write solution inventory and coverage without any model calls")
    ...
    if args.input and args.manifest:
        parser.error("--manifest applies to --runs; saved samples carry their metadata")
    ...
    if args.input:
        ...
        samples, issues = load_samples(args.input), []
    else:
    ...
    if not args.prepare_only:
        ...
        if args.input:
            ...
            extract_samples(samples, summarizer)
        ...
        scores, comparisons = [], []
    save_outputs(args.out, samples, issues, scores, comparisons, settings, identity, calls,
    ...
    failures = Counter(s.get("error", s.get("extraction_status")) for s in samples
                       if s.get("extraction_status") not in ("ok", "excluded"))
    if failures:
        print("Coverage gaps: " + "; ".join(f"{key}={count}" for key, count in failures.items()))
    if args.prepare_only:
        print("Inventory only. Complete solution snapshots are ready for extraction.")
        return 0
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"compare_solution_vendi: {exc}") from exc
