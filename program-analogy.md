...
The Job configures `ANALOGY_MODEL` and `ANALOGY_BASE_URL`, and injects `ANALOGY_API_KEY` from the same `mlevolve-llm-proxy` Secret (`LLM_API_KEY`) used by MLEvolve Jigsaw. Claude and its shell calls inherit this environment; no manual key entry is needed for new Jobs. Never print credentials or save them in run artifacts. If an older Pod lacks the key, report the missing configuration; a user-provided credential may be exported for each CLI call without changing the protocol. Use the existing default retrieval settings, with full-text reading enabled. Do not change the model, API settings, corpus, prepared split, dependencies, or protocol lock during the run. Use the fixed hardware description in `AUTORESEARCH_RESOURCES` for every call; do not put remaining time in `--resources`.
